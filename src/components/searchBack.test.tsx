...
it('chooses another symbol while prevention is still on', () => {
  const navigation = createNavigation();
  const prevented: boolean[] = [];
  const chosen: string[] = [];
  const wrapper = ({ children }: { children: ReactNode }) => (
  ...
  const { result } = renderHook(
    () => {
      ...
      useDismissSearchOnBack(open, () => setOpen(false));
      return (next: string) => {
        if (next === 'BTCUSDT') setOpen(false);
        else chosen.push(next);
      };
    },
    { wrapper },
  );
