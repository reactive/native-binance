import { formatAmount, formatClock, formatLast } from './formatMarket';
...
it('formats a BTCUSDT price to the tick and a size to the step', () => {
  ...
  expect(formatAmount(86473.85, '0.01000000')).toBe('86,473.85');
  expect(formatLast(0.002, '0.00001000')).toBe('0.00200');
  expect(formatAmount(0.002, '0.00001000')).toBe('0.00200');
});

it('uses the value decimals when the step has not loaded', () => {
