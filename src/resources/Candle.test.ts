...
import { Candle, CANDLE_LIMIT, getCandles, upsertCandle } from './Candle';
...
function setCandle(
  ...
  act(() => {
    promise = controller.set(upsertCandle, args, [value]);
  });
  return promise;
}

function storedList(
  controller: { getState: () => unknown },
  symbol: string,
  interval: string,
): unknown {
  ...
  return state.entities['[Candle]']?.[JSON.stringify({ interval, symbol })];
}

it('normalizes kline tuples into candles, oldest first', () => {
  const { result, controller } = renderDataHook(
    () => useSuspense(getCandles, { symbol: 'BTCUSDT', interval: '15m' as const }),
    {
      initialFixtures: [
        {
          endpoint: getCandles,
          args: [{ symbol: 'BTCUSDT', interval: '15m' }],
          response: three,
        },
      ],
    },
  ...
  expect(result.current[0]).toBeInstanceOf(Candle);
  expect(result.current[0].close).toBe(101.5);
  expect(typeof result.current[0].close).toBe('number');
  expect(result.current[0].pk()).toBe('BTCUSDT:15m:1000');
  expect(storedList(controller, 'BTCUSDT', '15m')).toEqual([
    'BTCUSDT:15m:1000',
    'BTCUSDT:15m:2000',
    'BTCUSDT:15m:3000',
  ]);
...
it('merges a forming candle in place and keeps the other entities', async () => {
  ...
  await setCandle(controller, { symbol: 'BTCUSDT', interval: '15m' }, kline(3_000, '109.5', 12));
...
it('appends a new open time and drops the oldest past the limit', async () => {
  ...
  await setCandle(controller, { symbol: 'BTCUSDT', interval: '15m' }, kline(4_000, '104', 1));
  ...
  const { result: capped, controller: cappedController } = renderDataHook(
    () => useSuspense(getCandles, { symbol: 'BTCUSDT', interval: '15m' as const }),
    {
      initialFixtures: [
        {
  ...
  await setCandle(
    ...
    kline((CANDLE_LIMIT + 1) * 1_000, '2', 1),
  );
  expect(capped.current).toHaveLength(CANDLE_LIMIT);
  expect(capped.current[0].openTime).toBe(2_000);
  expect(capped.current[CANDLE_LIMIT - 1].openTime).toBe((CANDLE_LIMIT + 1) * 1_000);
});
...
it('keeps the stored candle when an older trade count arrives', async () => {
  ...
  await setCandle(controller, { symbol: 'BTCUSDT', interval: '15m' }, kline(3_000, '1', 3));
